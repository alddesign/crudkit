...
class AdminPanelController extends Controller
{
	...
	private function getColumnsCode(array $columns)
	{
		$code = '';
	}
    ...
    {		
		...
		//Process Reqest Data
		$qrCode 				= $this->getQrCodeTag($request);
		$listPageUrl 			= $this->authHelper->userHasAccessTo(session('crudkit-username'), $pageId, 'list') ? URL::action('\Alddesign\Crudkit\Controllers\AdminPanelController@listView', ['page-id' => $pageId]) : '';
		$deleteUrl				= URL::action('\Alddesign\Crudkit\Controllers\AdminPanelController@deleteRecord', $this->getUrlParameters($pageId, null, null, null, null, $primaryKeyValues));
		$updateUrl				= URL::action('\Alddesign\Crudkit\Controllers\AdminPanelController@updateView', $this->getUrlParameters($pageId, null, null, null, null, $primaryKeyValues));
    ...
    {				
		...
		[
            ...
            'primaryKeyColumns' 		=> $pageDescriptor->getTable()->getPrimaryKeyColumns(true),
			'listPageUrl' 				=> $listPageUrl,
			'getChartDataUrl' 			=> URL::action('\Alddesign\Crudkit\Controllers\AdminPanelController@getChartData'),
    ...
    public function deleteRecord(Request $request)
    {
		...
		//Redirect
        return Response::redirectToAction('\Alddesign\Crudkit\Controllers\AdminPanelController@listView', ['page-id' => $pageId]);
    }
	...
	public function login(Request $request)
	{
		...
		return Response::redirectToAction('\Alddesign\Crudkit\Controllers\AdminPanelController@index');
	}
	...
	public function logout(Request $request)
	{
		...
		return Response::redirectToAction('\Alddesign\Crudkit\Controllers\AdminPanelController@loginView');
	}
	
	//Executes a predefined callback action for a specific page
	...
	private function getCardPageUrls(array $records, array $primaryKeyColumns, string $pageId, bool $singleRecord = false)
	{
		...
		foreach($records as $index => $record)
		{
			...
			$result[$index] = URL::action('\Alddesign\Crudkit\Controllers\AdminPanelController@cardView', $urlParameters);
		}
	...
	private function getExportCsvUrl(string $pageId, string $searchText = '', string $searchColumnName = '', array $filters = [])
	{
		...
		return(URL::action('\Alddesign\Crudkit\Controllers\AdminPanelController@exportRecordsCsv', $urlParameters));
	}
	
	private function getExportXmlUrl(string $pageId, string $searchText = '', string $searchColumnName = '', array $filters = [])
	{
		$urlParameters = $this->getUrlParameters($pageId,null,$searchText,$searchColumnName,$filters);
		return(URL::action('\Alddesign\Crudkit\Controllers\AdminPanelController@exportRecordsXml', $urlParameters));
	}
	
	private function getChartPageUrl(string $pageId, string $searchText = '', string $searchColumnName = '', array $filters = [])
	{
		$urlParameters = $this->getUrlParameters($pageId,null,$searchText,$searchColumnName,$filters);
		return(URL::action('\Alddesign\Crudkit\Controllers\AdminPanelController@chartView', $urlParameters));
	}
	...
	private function getPaginationUrls(int $pageNumber, string $pageId, int $recordCount, string $searchText = '', string $searchColumnName = '', array $filters = [])
	{
		...
		// ### Create Urls ###
		//Previous
		$result['previous'] = URL::action('\Alddesign\Crudkit\Controllers\AdminPanelController@listView', $params);
		...
		for($c = 1; (($c - 1) * $recordsPerPage) < $recordCount; $c++)
		{
			...
			{
				$result['pages'][$c] = URL::action('\Alddesign\Crudkit\Controllers\AdminPanelController@listView', $params);
			}
			
			if($c === 1) //First / Last
			{
				$result['first'] = URL::action('\Alddesign\Crudkit\Controllers\AdminPanelController@listView', $params);
			}
			if(($c * $recordsPerPage) >= $recordCount) //Last
			{
				$result['last'] = URL::action('\Alddesign\Crudkit\Controllers\AdminPanelController@listView', $params);
			}
		}
		
		//Next
		$params['page-number'] = $pageNumber + 1;
		$result['next'] = URL::action('\Alddesign\Crudkit\Controllers\AdminPanelController@listView', $params);
     ...
     */
	private static function getHtmlInputAttributes($columns)
	{
		$htmlInputAttributes = [];
